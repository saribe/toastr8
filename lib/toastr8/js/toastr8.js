    ...
    define(['jquery'], function ($) {
        return (function () {
            ...
            function notify(map) {
                ...
                /// <param name="iconClass" type="Object">Css Class to vendor logo on right</param>
                ...
                var $toastElement = $('<table/>')
                    ...
                    .find("tr");
                var $messageContainer = $("<td class='toast8-message-container'></td>");
                ...
                if (map.imgClass) {
                    ...
                    var style = "style=\"background-size: 100% 100%;";
                    for (var i = 0, k = map.imgURI.length; i < k; i++) {
                        style += "background-image: url('" + map.imgURI[i] + "');";
                    }
                    style += "\"";
                    $toastElement.append("<td class='toast8-avatar-container '" + style + "></td>");
                }
            ...
            function getOptions() {
                return $.extend({}, getDefaults(), vm.options);
            }
            function getDefaults() {
                return {
                    ...
                    target: 'body',
                    closeHtml: "<b class='toast8-close-button'>×</b>",
                    newestOnTop: true
                };
            }
            ...
            function facebook() {
                return notify(unwrapArguments(arguments, "facebook", getOptions().iconClasses.facebook));
            }
            function info() {
                return notify(unwrapArguments(arguments, "info", getOptions().iconClasses.info));
            }
            function error() {
                return notify(unwrapArguments(arguments, "error", getOptions().iconClasses.error));
            }
            function warning() {
                return notify(unwrapArguments(arguments, "warning", getOptions().iconClasses.warning));
            }
            function success() {
                return notify(unwrapArguments(arguments, "success", getOptions().iconClasses.success));
            }
            function twitter() {
                return notify(unwrapArguments(arguments, "twitter", getOptions().iconClasses.twitter));
            }
            function linkedin() {
                return notify(unwrapArguments(arguments, "linkedin", getOptions().iconClasses.linkedin));
            }
            function windows() {
                return notify(unwrapArguments(arguments, "windows", getOptions().iconClasses.windows));
            }
            function googlePlus() {
                return notify(unwrapArguments(arguments, "google-plus", getOptions().iconClasses.googlePlus));
            }
            function skype() {
                return notify(unwrapArguments(arguments, "skype", getOptions().iconClasses.skype));
            }
            function linux() {
                return notify(unwrapArguments(arguments, "linux", getOptions().iconClasses.linux));
            }
            function github() {
                return notify(unwrapArguments(arguments, "github", getOptions().iconClasses.github));
            }
            function android() {
                return notify(unwrapArguments(arguments, "android", getOptions().iconClasses.android));
            }
